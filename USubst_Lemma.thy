...
next
  ...
  have  NOU:"NOUadmit \<sigma> ODE (ODE_vars ODE)"
    ...
    by auto
          (*have mkv_eq':"\<And>t. mk_v I (NOsubst ODE \<sigma>) (sol 0, b) (sol t) = mk_v I (NOsubst ODE \<sigma>) (sol 0, b) (sol 0)"
            subgoal for t
              apply (rule agree_UNIV_eq)
              using mk_v_agree [of I "(NOsubst ODE \<sigma>)" "(sol 0, b)" "(sol t)"]
              using mk_v_agree [of I "(NOsubst ODE \<sigma>)" "(sol 0, b)" "(sol 0)"]
              unfolding Vagree_def
              apply auto
              subgoal for i
                apply (erule allE[where x=i])+
          ...
          (*have fml_eq:"\<And>t. (mk_v I (NOsubst ODE \<sigma>) (sol 0, b) (sol t) \<in> fml_sem I (NFsubst \<phi> \<sigma>)) 
            = (mk_v (NTadjoint I \<sigma> (sol 0, b)) ODE (sol 0, b) (sol t) \<in> fml_sem (NTadjoint I \<sigma> (sol 0, b)) \<phi>)"
    (*and IH:"fsafe \<phi> \<Longrightarrow> (\<And>i. dfree (\<sigma> i)) \<Longrightarrow> (\<And>\<nu>. (\<nu> \<in> fml_sem I (NFsubst \<phi> \<sigma>)) = (\<nu> \<in> fml_sem (NTadjoint I \<sigma> \<nu>) \<phi>))"*)
            subgoal for t
              using IH[of "(sol 0,b)", OF fsafe frees]
                mkv_eq'[of t] 
      ...
      show "((\<nu>, \<omega>) \<in> prog_sem I (NPsubst (EvolveODE ODE \<phi>) \<sigma>)) = ((\<nu>, \<omega>) \<in> prog_sem (NTadjoint I \<sigma> \<nu>) (EvolveODE ODE \<phi>))"
        proof (auto simp del: prog_sem.simps(8) simp add: ode_alt_sem[OF osafe' fsafe'] ode_alt_sem[OF osafe fsafe])
          fix a b 
            ...
            and t :: real
          assume eq1:"\<nu> = (a, b)"
          assume eq2:"\<omega> = mk_v I (NOsubst ODE \<sigma>) (a, b) (sol t)"
          assume t:"0 \<le> t"
          assume sol:"(sol solves_ode (\<lambda>a. ODE_sem I (NOsubst ODE \<sigma>))) {0..t} 
             {x. mk_v I (NOsubst ODE \<sigma>) (a, b) x \<in> fml_sem I (NFsubst \<phi> \<sigma>)}"
          assume VSA:"VSagree (sol 0) a {x. Inl x \<in> ODE_vars (NOsubst ODE \<sigma>) \<or> Inl x \<in> FVO (NOsubst ODE \<sigma>) \<or> Inl x \<in> FVF (NFsubst \<phi> \<sigma>)}"
          (* Potentially useful, not necessary *)
          then have mkv:"mk_v (NTadjoint I \<sigma> (sol 0, b)) ODE (sol 0, b) (sol 0)  = mk_v (NTadjoint I \<sigma> (sol 0, b)) ODE (sol 0, b) (sol t)"
            sorry
          then have mkv_eq':"\<And>t. mk_v I (NOsubst ODE \<sigma>) (sol 0, b) (sol 0) = mk_v (NTadjoint I \<sigma> (sol 0, b)) ODE (sol 0, b) (sol t)"
            sorry
          ...
          (* Proven and possibly useful *) 
          have mkv1:"mk_v I (NOsubst ODE \<sigma>) (sol t, b) (fst (sol t, b)) = mk_v (NTadjoint I \<sigma> (sol t,b)) ODE (sol t, b) (fst (sol t, b))"
            apply (rule nsubst_mkv) 
            using NOU good_interp osafe frees by auto
          have mkv2:"mk_v I (NOsubst ODE \<sigma>) (sol 0, b) (fst (sol 0, b)) = mk_v (NTadjoint I \<sigma> (sol 0, b)) ODE (sol 0, b) (fst (sol 0, b))"
            apply (rule nsubst_mkv[of I \<sigma> ODE ])
            subgoal by (rule good_interp)
            defer
            subgoal by (rule osafe)
            subgoal by (rule frees)
            by (rule NOU)
          then have mkv3:"mk_v I (NOsubst ODE \<sigma>) (sol 0, b) (sol 0) = mk_v (NTadjoint I \<sigma> (sol 0, b)) ODE (sol 0, b) (sol 0)"
            by auto
          (* Necessary *)
          ...
          have "\<And>t. mk_v (NTadjoint I \<sigma> (sol t, b)) ODE  = mk_v (NTadjoint I \<sigma> (a, b)) ODE"
            apply (rule uadmit_mkv_ntadjoint)
            prefer 3
            subgoal for t  using NOU hmm[of t] unfolding NOUadmit_def Vagree_def by fastforce   
            using frees good_interp osafe by auto
          then have mkva:"\<And>t. mk_v (NTadjoint I \<sigma> (sol t, b)) ODE (a, b) (sol t) = mk_v (NTadjoint I \<sigma> (a, b)) ODE (a, b) (sol t)"
            by auto
          have main_eq:"\<And>t. mk_v I (NOsubst ODE \<sigma>) (a, b) (sol t) = mk_v (NTadjoint I \<sigma> (a, b)) ODE (a, b) (sol t) "
            using mkv mkva by auto
          note mkvt = main_eq[of t]
          have fml_eq1:"\<And>t. 
              (mk_v I (NOsubst ODE \<sigma>) (a, b) (sol t) \<in> fml_sem I (NFsubst \<phi> \<sigma>)) 
            = (mk_v I (NOsubst ODE \<sigma>) (a, b) (sol t) \<in> fml_sem (NTadjoint I \<sigma> (mk_v I (NOsubst ODE \<sigma>) (a, b) (sol t))) \<phi>)"
            using IH[OF fsafe frees] by auto
          moreover have fml_eq2:" ... = (\<lambda>t. mk_v I (NOsubst ODE \<sigma>) (a, b) (sol t) \<in> fml_sem (NTadjoint I \<sigma> (a, b)) \<phi>)"
            apply(rule ext)
            subgoal for t
              by (metis NFU frees fsafe good_interp mk_v_agree osubst_preserves_ODE_vars uadmit_fml_ntadjoint)
            done
          moreover have fml_eq3:" ... = (\<lambda>t. mk_v (NTadjoint I \<sigma> (a,b)) ODE (a, b) (sol t) \<in> fml_sem (NTadjoint I \<sigma> (a, b)) \<phi>) "
            using main_eq by auto
          ultimately have fml_eq: 
            "\<And>t. (mk_v I (NOsubst ODE \<sigma>) (a, b) (sol t) \<in> fml_sem I (NFsubst \<phi> \<sigma>)) 
              =  (mk_v (NTadjoint I \<sigma> (a,b)) ODE (a, b) (sol t) \<in> fml_sem (NTadjoint I \<sigma> (a, b)) \<phi>)"
             by meson
          have sem_eq:"(\<lambda>a. ODE_sem I (NOsubst ODE \<sigma>)) = (\<lambda>a. ODE_sem (NTadjoint I \<sigma> (a, b)) ODE)"
            sorry
          have vsub:" 
            ...
            sorry
          have sol':"(sol solves_ode (\<lambda>_. ODE_sem (NTadjoint I \<sigma> (a, b)) ODE)) {0..t}
             {x. mk_v I (NOsubst ODE \<sigma>) (a, b) x \<in> fml_sem I (NFsubst \<phi> \<sigma>)}"
            (* using sol unfolding solves_ode_def *)
            apply (rule solves_ode_congI)
            apply (rule sol)
            subgoal for ta by auto
            subgoal for ta using sem_eq by metis
            subgoal by (rule refl)
            subgoal by (rule refl)
            done
          have sub:"\<And>s. s \<in> {0..t} 
                  \<Longrightarrow> sol s \<in> {x. (mk_v (NTadjoint I \<sigma> (a,b)) ODE (a, b) x \<in> fml_sem (NTadjoint I \<sigma> (a, b)) \<phi>)}"
            using fml_eq rangeI t sol solves_ode_domainD by fastforce
          have sol'':"(sol solves_ode (\<lambda>c. ODE_sem (NTadjoint I \<sigma> (a, b)) ODE)) {0..t}
     {x. mk_v (NTadjoint I \<sigma> (a, b)) ODE (a, b) x \<in> fml_sem (NTadjoint I \<sigma> (a, b)) \<phi>}"
            apply (rule solves_odeI)
            subgoal using sol' solves_ode_vderivD by blast
            using sub by auto
          show"\<exists>sola ta. mk_v I (NOsubst ODE \<sigma>) (a, b) (sol t) = mk_v (NTadjoint I \<sigma> (a, b)) ODE (a, b) (sola ta) \<and>
                 0 \<le> ta \<and>
                 (sola solves_ode (\<lambda>c. ODE_sem (NTadjoint I \<sigma> (a, b)) ODE)) {0..ta}
                  {x. mk_v (NTadjoint I \<sigma> (a, b)) ODE (a, b) x \<in> fml_sem (NTadjoint I \<sigma> (a, b)) \<phi>} \<and>
                 VSagree (sola 0) a {x. Inl x \<in> ODE_vars ODE \<or> Inl x \<in> FVO ODE \<or> Inl x \<in> FVF \<phi>}"
            apply(rule exI[where x=sol])
            apply(rule exI[where x=t])
            apply(rule conjI)
            subgoal by (rule mkvt)
            apply(rule conjI)
            subgoal by (rule t)
            apply(rule conjI)
            subgoal by (rule sol'')
            subgoal (*using VSagree_sub[OF vsub]  VSA apply auto*) sorry 
            done
      next
        fix b 
          and sol::"real \<Rightarrow> (real, 'sz) vec" 
        ...
        show "((sol 0, b), mk_v (NTadjoint I \<sigma> (sol 0, b)) ODE (sol 0, b) (sol t)) \<in> prog_sem I (EvolveODE (NOsubst ODE \<sigma>) (NFsubst \<phi> \<sigma>))"
          sorry
        qed
   ...
   case (Fadmit_Prop1 \<sigma> args p p') 
      ...
      proof -
        ...
        have eqs:"\<And>i \<nu>'. dterm_sem I (Tsubst (args i) \<sigma>) \<nu> = dterm_sem (adjoint I \<sigma> \<nu>) (args i) \<nu>"
          by (auto simp add: IH safes)
        let ?sub = "(\<lambda> i. Tsubst (args i) \<sigma>)"
        have subFree:"(\<forall> i. dfree (?sub i))"
          using tsubst_preserves_free[OF frees ssafes(1)]
          by (simp add: frees ssafes tsubst_preserves_free)
        have freef:"fsafe p'" using ssafe some unfolding ssafe_def by auto 
        have IH2:"(\<nu> \<in> fml_sem I (NFsubst p' ?sub)) = (\<nu> \<in> fml_sem (NTadjoint I ?sub \<nu>) p')"
