text\<open> Interval-Arithmetic.thy implements conservative interval arithmetic computations over a simple
   programming language where all terms have 32-bit word values, with explicit infinities for terms
...
text\<open>Sentinel values for infinities. Note that we leave the maximum value ($2^31$) completed unused,
  so that negation of $(2^31)-1$ is not an edge case\<close>
...
text\<open>Our statement language is nondeterministic first-order regular programs.
 This coincides with the discrete subset of hybrid programs from the dL entry.\<close>
text\<open>Our assertion language are the formulas of first-order dynamic logic\<close>
datatype hp =
...
where 
  rGreaterT:"\<lbrakk>([\<theta>\<^sub>1]\<nu> \<down> r\<^sub>1); ([\<theta>\<^sub>2]\<nu> \<down> r\<^sub>2)\<rbrakk> \<Longrightarrow> r\<^sub>1 > r\<^sub>2 \<Longrightarrow> ([Greater \<theta>\<^sub>1 \<theta>\<^sub>2] \<nu> \<downharpoonright> True)"
| rGreaterF:"\<lbrakk>([\<theta>\<^sub>1]\<nu> \<down> r\<^sub>1); ([\<theta>\<^sub>2]\<nu> \<down> r\<^sub>2)\<rbrakk> \<Longrightarrow> r\<^sub>2 \<ge> r\<^sub>1 \<Longrightarrow> ([Greater \<theta>\<^sub>1 \<theta>\<^sub>2] \<nu> \<downharpoonright> False)"
| rGeqT:"\<lbrakk>([\<theta>\<^sub>1]\<nu> \<down> r\<^sub>1); ([\<theta>\<^sub>2]\<nu> \<down> r\<^sub>2)\<rbrakk> \<Longrightarrow> r\<^sub>1 \<ge> r\<^sub>2 \<Longrightarrow> ([Geq \<theta>\<^sub>1 \<theta>\<^sub>2] \<nu> \<downharpoonright> True)"
| rGeqF:"\<lbrakk>([\<theta>\<^sub>1]\<nu> \<down> r\<^sub>1); ([\<theta>\<^sub>2]\<nu> \<down> r\<^sub>2)\<rbrakk> \<Longrightarrow> r\<^sub>2 > r\<^sub>1 \<Longrightarrow> ([Geq \<theta>\<^sub>1 \<theta>\<^sub>2] \<nu> \<downharpoonright> False)"
...
section \<open>Trivial arithmetic facts\<close>
...
subgoal 
  ...
  subgoal
    ...
    have scast_eq3:"sint((scast w\<^sub>2)::64 Word.word) = sint w\<^sub>2"
      apply(rule Word.sint_up_scast)
      unfolding Word.is_up by auto
    have w2Geq:"sint ((scast w\<^sub>2)::64 Word.word) \<ge> - (2 ^ 31) " 
      using Word.word_sint.Rep[of "(w\<^sub>2)::32 Word.word"] sints32 Word.word_size scast_eq1 upcast_max
        scast_eq3 len32 mem_Collect_eq 
      by auto
    have "sint ((scast w\<^sub>2)::64 Word.word) \<le> 2 ^ 31"
    ...
    then have w2Less:"sint ((scast w\<^sub>2)::64 Word.word) < 9223372039002259455" by auto
    have w2Range:
      "-(2 ^ (size ((scast w\<^sub>2)::64 Word.word) - 1)) 
       \<le> sint ((scast w\<^sub>2)::64 Word.word) + sint ((-0x7FFFFFFF)::64 Word.word) 
     \<and> sint ((scast w\<^sub>2)::64 Word.word) + sint ((-0x7FFFFFFF)::64 Word.word) 
       \<le> 2 ^ (size ((scast w\<^sub>2)::64 Word.word) - 1) - 1"
      apply(auto simp add: Word.word_size scast_eq1 upcast_max sints64 max_less)
      using max_diff_pos max_less w2Less w2Geq by auto
    have case1a:"sint (((scast w\<^sub>2)::64 Word.word) + (-0x7FFFFFFF::64 Word.word)) 
              =  sint ((scast w\<^sub>2)::64 Word.word) + sint (-0x7FFFFFFF::64 Word.word)"
    ...
    have case1:"?sum <=s ((scast NEG_INF)::64 Word.word) \<Longrightarrow> NEG_INF \<equiv>\<^sub>U r\<^sub>1 + r\<^sub>2"
      ...
      apply(auto)
          ...
          apply (auto simp add: NEG_INF_def neq1 eq2 neq3 repINT repU_def repe.simps repeInt_simps
          up2 word_sless_alt) 
      using repINT repU_def repe.simps repeInt_simps up2 word_sless_alt POS_INF_def NEG_INF_def 
      ...
      subgoal
        proof -
         ...
         have case1a:" sint (((scast w\<^sub>2)::64 Word.word) + (-0x7FFFFFFF::64 Word.word)) 
                   =  sint ((scast w\<^sub>2)::64 Word.word) + sint (-0x7FFFFFFF::64 Word.word)"
           by(rule signed_arith_sint(1)[OF w2Range])
         have a1:"sint (((scast w\<^sub>2)::64 Word.word) + (-0x7FFFFFFF)) 
                = sint((scast w\<^sub>2)::64 Word.word) + sint((-0x7FFFFFFF)::64 Word.word)" 
         ...
         have "- 0x7FFFFFFF < sint w\<^sub>2 + (- 0x7FFFFFFF)"
           ...
           using min_extend_val' scast_eq3 by linarith
         ...
         have rightSize:"sint (((scast w\<^sub>2)::64 Word.word) + - 0x7FFFFFFF) \<in> sints (len_of TYPE(32))"
           unfolding case1a b1 min_extend_val'
        ...
        have downcast:"sint ((scast (((scast w\<^sub>2)::64 Word.word) + ((- 0x7FFFFFFF))))::word) 
                     = sint (((scast w\<^sub>2)::64 Word.word) + ((- 0x7FFFFFFF)::64 Word.word)) "
          using scast_down_range[OF rightSize]
          by auto
        then have b:"sint ((scast (((scast w\<^sub>2)::64 Word.word) + 0xFFFFFFFF80000001))::word)
                   = sint (((scast w\<^sub>2)::64 Word.word) + 0xFFFFFFFF80000001)" 
          using min_extend_val by auto
        have c:"sint (((scast w\<^sub>2)::64 Word.word) + 0xFFFFFFFF80000001) 
              = sint ((scast w\<^sub>2)::64 Word.word) + sint ((-0x7FFFFFFF)::64 Word.word)"
        ...
        show "r'\<^sub>2 - 2147483647 
          = (real_of_int (sint ((scast (((scast w\<^sub>2)::64 Word.word) + 0xFFFFFFFF80000001))::word)))"
          using a b c min_extend_val' e f 
          proof -
            have "(real_of_int (sint ((scast w\<^sub>2)::64 Word.word) - 2147483647)) 
                = r'\<^sub>2 - (real_of_int 2147483647)"
              by (simp add: e leq2)
            then show ?thesis
              using  b c min_extend_val' by(auto)
          qed
        qed
      subgoal  
      proof -
        have range2a:" - (2 ^ (size ((scast w\<^sub>2)::64 Word.word) - 1)) 
          \<le> sint ((scast w\<^sub>2)::64 Word.word) + sint ((-0x7FFFFFFF)::64 Word.word) 
        \<and> sint ((scast w\<^sub>2)::64 Word.word) + sint ((-0x7FFFFFFF)::64 Word.word) 
          \<le> 2 ^ (size ((scast w\<^sub>2)::64 Word.word) - 1) - 1"
        ...
        have case2a:"sint (((scast w\<^sub>2)::64 Word.word) + (-0x7FFFFFFF::64 Word.word)) 
                  =  sint ((scast w\<^sub>2)::64 Word.word) + sint (-0x7FFFFFFF::64 Word.word)"
          by(rule signed_arith_sint(1)[OF range2a])
        have neg64:"(((scast w\<^sub>2)::64 Word.word) + 0xFFFFFFFF80000001) 
                  = ((scast w\<^sub>2)::64 Word.word) + (-0x7FFFFFFF)" by auto
        assume "\<not> sint (((scast w\<^sub>2)::64 Word.word) + 0xFFFFFFFF80000001) \<le> - 2147483647"
        then have sintw2_bound:"sint (((scast w\<^sub>2)::64 Word.word) + (-0x7FFFFFFF)) > - 2147483647"
          unfolding neg64 by auto 
        have a:"sint (((scast w\<^sub>2)::64 Word.word) + (-0x7FFFFFFF)) 
              = sint((scast w\<^sub>2)::64 Word.word) + sint((-0x7FFFFFFF)::64 Word.word)" 
        ...
        have rightSize:"sint (((scast w\<^sub>2)::64 Word.word) + - 0x7FFFFFFF) \<in> sints (len_of TYPE(32))"
          ...
          subgoal 
            using Word.word_sint.Rep[of "(w\<^sub>2)::32 Word.word"] 
            by (auto simp add: sints32 len32)
          using w2bound by auto
        have downcast:"sint ((scast (((scast w\<^sub>2)::64 Word.word) + ((- 0x7FFFFFFF))))::word) 
                    = sint (((scast w\<^sub>2)::64 Word.word) + ((- 0x7FFFFFFF)::64 Word.word))"
          using scast_down_range[OF rightSize]
        ...
        have "sint (scast (((scast w\<^sub>2)::64 Word.word) + (-0x7FFFFFFF))::word) < 2147483647"
          ...
          using range2s[of "sint w\<^sub>2", OF d]
          by auto
  ...
  subgoal
    ...
    have thing2:"sint ((scast w\<^sub>1)::64 Word.word) \<ge> - (2 ^ 31) " 
      using Word.word_sint.Rep[of "(w\<^sub>1)::32 Word.word"] sints32 Word.word_size scast_eq1 scast_eq2 
        scast_eq3 len32 mem_Collect_eq 
      by auto
    ...
    have truth:" - (2 ^ (size ((scast w\<^sub>1)::64 Word.word) - 1)) 
      \<le> sint ((scast w\<^sub>1)::64 Word.word) + sint ((-0x7FFFFFFF)::64 Word.word) 
    \<and> sint ((scast w\<^sub>1)::64 Word.word) + sint ((-0x7FFFFFFF)::64 Word.word) 
      \<le> 2 ^ (size ((scast w\<^sub>1)::64 Word.word) - 1) - 1"
      apply(auto simp add:
      Word.word_size[of "(scast w\<^sub>1)::64 Word.word"] 
    ...
    have case1a:"sint (((scast w\<^sub>1)::64 Word.word) + (-0x7FFFFFFF::64 Word.word)) 
               = sint ((scast w\<^sub>1)::64 Word.word) + sint (-0x7FFFFFFF::64 Word.word)"
      by (rule signed_arith_sint(1)[of "(scast w\<^sub>1)::64 Word.word" "(- 0x7FFFFFFF)", OF truth])
    have c:"sint (((scast w\<^sub>1)::64 Word.word) + 0xFFFFFFFF80000001) 
          = sint ((scast w\<^sub>1)::64 Word.word) + sint ((-0x7FFFFFFF)::64 Word.word)"
      using min_extend_val case1a by auto
    ...
    have neg64:"(((scast w\<^sub>1)::64 Word.word) + 0xFFFFFFFF80000001) 
              = ((scast w\<^sub>1)::64 Word.word) + (-0x7FFFFFFF)" 
        by auto
    have case2:"\<not>(?sum <=s scast NEG_INF) \<Longrightarrow> scast ?sum \<equiv>\<^sub>U r\<^sub>1 + r\<^sub>2"
      apply (simp add: repU_def NEG_INF_def repe.simps POS_INF_def word_sle_def up1 up2)
      ...
      subgoal
        ...
        have arith:"\<And>x::int. x \<le> 2 ^ 31 - 1 \<Longrightarrow> x + (- 2147483647) < 2147483647"
          by auto
        have downcast:"sint ((scast (((scast w\<^sub>1)::64 Word.word) + ((- 0x7FFFFFFF))))::word)
                     = sint (((scast w\<^sub>1)::64 Word.word) + ((- 0x7FFFFFFF)::64 Word.word))"
          using scast_down_range[OF rightSize]
          by auto
        then have b:"sint ((scast (((scast w\<^sub>1)::64 Word.word) + 0xFFFFFFFF80000001))::word) 
                   = sint (((scast w\<^sub>1)::64 Word.word) + 0xFFFFFFFF80000001)" 
          using min_extend_val by auto
        have "(real_of_int (sint ((scast w\<^sub>1)::64 Word.word ) - 2147483647)) 
            = r'\<^sub>1 -  (real_of_int 2147483647)"
            by (simp add: e leq2)
        then show "r'\<^sub>1 -  2147483647 
          = (real_of_int (sint ((scast (((scast w\<^sub>1)::64 Word.word) + 0xFFFFFFFF80000001))::word)))"
      ...
      proof -
        ...
        have rightSize:"sint (((scast w\<^sub>1)::64 Word.word) + - 0x7FFFFFFF) \<in> sints (len_of TYPE(32))"
          ...
          apply(auto simp add: sints32 len32[of "TYPE(32)"])
          defer
          subgoal using Word.word_sint.Rep[of "(w\<^sub>1)::32 Word.word"] 
            by (auto simp add: sints32 len32[of "TYPE(32)"])
          using w2bound by auto
        have arith:"\<And>x::int. x \<le> 2 ^ 31 - 1 \<Longrightarrow> x + (- 2147483647) < 2147483647"
          by auto
        ...
        show "sint (scast (((scast w\<^sub>1)::64 Word.word) + 0xFFFFFFFF80000001)::word) < 2147483647"
          using downcast min_extend_val' e c arith[of "sint w\<^sub>1", OF d2]
          by auto
      qed
      subgoal proof -
...
proof -
  ...
  have sints32:"sints 32 = {i. - (2 ^ 31) \<le> i \<and> i < 2 ^ 31}"
    using sints_def[of 32] range_sbintrunc[of 31] by auto 
  have truth:" - (2 ^ (size ((scast w\<^sub>1)::64 Word.word) - 1)) 
     \<le> sint ((scast w\<^sub>1)::64 Word.word) + sint ((scast w\<^sub>2)::64 Word.word) 
   \<and> sint ((scast w\<^sub>1)::64 Word.word) + sint ((scast w\<^sub>2)::64 Word.word) 
     \<le> 2 ^ (size ((scast w\<^sub>1)::64 Word.word) - 1) - 1"
  ...
  have sint_eq:"sint((scast w\<^sub>1 + scast w\<^sub>2)::64 Word.word) = sint w\<^sub>1 + sint w\<^sub>2"
    ...
    using scast_eq1 scast_eq2
    by auto
  have bigOne:"scast w\<^sub>1 + scast w\<^sub>2 <=s ((- 0x7FFFFFFF)::64 Word.word) 
    \<Longrightarrow> \<exists>r'\<ge>r\<^sub>1 + r\<^sub>2. r' \<le>  (- 0x7FFFFFFF)"
   proof -
     ...
     have somethingA:"r'\<^sub>1  \<le>  sint w\<^sub>1" and somethingB:"r'\<^sub>2  \<le>  sint w\<^sub>2" 
       ...
       using \<open>scast w\<^sub>1 + scast w\<^sub>2 <=s - 0x7FFFFFFF\<close>  word_sle_def notinf1 notinf2 
       unfolding POS_INF_def
       by auto
   ...
   "\<not>(((scast POS_INF)::64 Word.word) <=s ?sum) \<Longrightarrow> 
    ...
    \<exists>r'\<ge>r\<^sub>1 + r\<^sub>2. r' = 
      (real_of_int (sint (scast (((scast w\<^sub>1)::64 Word.word) + ((scast w\<^sub>2)::64 Word.word))::word))) 
   ...
   proof -
     ...
     then have
       ...
       unfolding repe.simps
       using notinf1 notinf2 notneginf1 notneginf2 by (auto simp add: POS_INF_def NEG_INF_def)
     have "sint (((scast w\<^sub>1)::64 Word.word) + ((scast w\<^sub>2)::64 Word.word)) 
         = sint ((scast (((scast w\<^sub>1)::64 Word.word) + ((scast w\<^sub>2)::64 Word.word)))::word)"
       apply(rule scast_down_range)
       unfolding sint_eq using sints32 sum_geq sum_leq by auto
     then have cast_eq:"(sint ((scast (((scast w\<^sub>1)::64 Word.word) 
                      + ((scast w\<^sub>2)::64 Word.word)))::word)) 
                    = sint w\<^sub>1 + sint w\<^sub>2"
     ...
     have r12_sint_scast:"r'\<^sub>1 + r'\<^sub>2 
       = (sint ((scast (((scast w\<^sub>1)::64 Word.word) 
         + ((scast w\<^sub>2)::64 Word.word)))::word))"
   ...
   have int_case:"\<not>(((scast POS_INF)::64 Word.word) <=s ?sum) 
              ...
              \<Longrightarrow> ((scast ?sum)::word) \<equiv>\<^sub>U r\<^sub>1 + r\<^sub>2"
   proof -
     ...
     obtain r'::real  
       where rDef:"r' =  (real_of_int (sint ((scast (((scast w\<^sub>1)::64 Word.word) 
                         + ((scast w\<^sub>2)::64 Word.word)))::word)))"
     ...
     show ?thesis
       ...
       by auto 
   qed
   show "(let sum::64 Word.word = scast w\<^sub>1 + scast w\<^sub>2 in 
     if scast POS_INF <=s sum then POS_INF 
     else if sum <=s scast NEG_INF then NEG_INF 
    else scast sum) \<equiv>\<^sub>U r\<^sub>1 + r\<^sub>2"
   ...
   subgoal 
     ...
     proof -
       ...
       then have "\<not> (scast w\<^sub>1::64 Word.word) + scast w\<^sub>2 <=s scast NEG_INF 
                \<and> \<not> (scast POS_INF::64 Word.word) <=s scast w\<^sub>1 + scast w\<^sub>2 
                \<and> \<not> (scast w\<^sub>1::64 Word.word) + scast w\<^sub>2 <=s scast NEG_INF 
             \<or> ((let w = scast w\<^sub>1 + scast w\<^sub>2 in 
                if scast POS_INF <=s (w::64 Word.word)  then POS_INF 
                else if w <=s scast NEG_INF then NEG_INF 
                else scast w) \<equiv>\<^sub>U r\<^sub>1 + r\<^sub>2)"
   ...
   proof -
     ...
     have "\<not> (scast w\<^sub>1::64 Word.word) + scast w\<^sub>2 <=s scast NEG_INF 
         \<and> \<not> (scast POS_INF::64 Word.word) <=s scast w\<^sub>1 + scast w\<^sub>2 
      \<or> ((let w = scast w\<^sub>1 + scast w\<^sub>2 in 
         if scast POS_INF <=s (w::64 Word.word) then POS_INF 
         else if w <=s scast NEG_INF then NEG_INF 
...
subgoal
  ...
  subgoal
    ...
    have "sint (( w\<^sub>2)) \<ge> (-(2 ^ 31))"
      using Word.word_sint.Rep[of "(w\<^sub>2)::32 Word.word"] sints32 mem_Collect_eq
      Word.word_size[of "(scast w\<^sub>2)::64 Word.word"] scast_eq1 scast_eq2 scast_eq3 len32 
      by auto
    ...
    have truth:" - (2 ^ (size ((scast w\<^sub>2)::64 Word.word) - 1)) 
        \<le> sint ((scast w\<^sub>2)::64 Word.word) + sint ((0x7FFFFFFF)::64 Word.word) 
      \<and> sint ((scast w\<^sub>2)::64 Word.word) + sint ((0x7FFFFFFF)::64 Word.word) 
        \<le> 2 ^ (size ((scast w\<^sub>2)::64 Word.word) - 1) - 1"
      by(auto simp add:
      Word.word_size[of "(scast w\<^sub>2)::64 Word.word"] 
      Word.word_size[of "(scast (0x7FFFFFFF))::64 Word.word"]
      scast_eq1 scast_eq2 
    ...
    have case1:" (((scast POS_INF)::64 Word.word) <=s ?sum) \<Longrightarrow> POS_INF \<equiv>\<^sub>L r\<^sub>1 + r\<^sub>2"
      ...
      apply(auto)
          using case1a case1b 
          apply (auto simp add: NEG_INF_def POS_INF_def neq1 eq2 neq3 repINT repL_def repe.simps 
            repeInt_simps lo2 word_sless_alt)
      using repINT repL_def repe.simps repeInt_simps lo2 word_sless_alt NEG_INF_def POS_INF_def 
        neq1 eq2 neq3 repINT repL_def repe.simps repeInt_simps lo2 word_sless_alt 
    ...
    have case2:"\<not>(scast POS_INF <=s ?sum) \<Longrightarrow> scast ?sum \<equiv>\<^sub>L r\<^sub>1 + r\<^sub>2"
      ...
      subgoal
        ...
        have downcast:"sint ((scast (((scast w\<^sub>2)::64 Word.word) + (( 0x7FFFFFFF))))::word) 
          ...
          using scast_down_range[OF rightSize]
          by auto
        then have b:"sint ((scast (((scast w\<^sub>2)::64 Word.word) + 0x7FFFFFFF))::word) 
                   = sint (((scast w\<^sub>2)::64 Word.word) + 0x7FFFFFFF)" 
          by auto
        have c:"sint (((scast w\<^sub>2)::64 Word.word) + 0x7FFFFFFF) 
              = sint ((scast w\<^sub>2)::64 Word.word) + sint ((0x7FFFFFFF)::64 Word.word)"
          using case1a by auto
        ...
        have f:"r'\<^sub>2 =  (real_of_int (sint w\<^sub>2))"
          by (simp add: leq2)
        show "r'\<^sub>2 +  2147483647 
          = (real_of_int (sint ((scast (((scast w\<^sub>2)::64 Word.word) + 0x7FFFFFFF))::word)))"
          using a b c d e f 
          proof -
            have "(real_of_int (sint ((scast w\<^sub>2)::64 Word.word ) + 2147483647)) 
                  = r'\<^sub>2 +  (real_of_int 2147483647)"
      ...
      subgoal  
      proof -
        have truth2a:"-(2^(size ((scast w\<^sub>2)::64 Word.word)-1)) 
            \<le> sint ((scast w\<^sub>2)::64 Word.word) + sint ((0x7FFFFFFF)::64 Word.word) 
          \<and> sint ((scast w\<^sub>2)::64 Word.word) + sint ((0x7FFFFFFF)::64 Word.word) 
            \<le> 2 ^ (size ((scast w\<^sub>2)::64 Word.word) - 1) - 1"
          apply(auto simp add:
        ...
        have min_cast:"(0x7FFFFFFF::64 Word.word) =((scast (0x7FFFFFFF::word))::64 Word.word)"
          by auto
        have neg64:"(((scast w\<^sub>2)::64 Word.word) + 0x7FFFFFFF) 
                  = ((scast w\<^sub>2)::64 Word.word) + (0x7FFFFFFF)"
          by auto
        ...
        have rightSize:"sint (((scast w\<^sub>2)::64 Word.word) +  0x7FFFFFFF) \<in> sints (len_of TYPE(32))"
          unfolding case2a b c 
          apply (auto simp add: sints32 len32[of "TYPE(32)"])
          using Word.word_sint.Rep[of "(w\<^sub>2)::32 Word.word"] sints32 len32[of "TYPE(32)"] w2bound 
          by auto
...
subgoal
  ...
  proof -
    ...
    have "sint (( w\<^sub>1)) \<ge> (-(2 ^ 31))"
      using Word.word_sint.Rep[of "(w\<^sub>1)::32 Word.word"] Word.word_size[of "(scast w\<^sub>1)::64 Word.word"]
        sints32 scast_eq1 scast_eq2 scast_eq3 len32 mem_Collect_eq
      by auto
    ...
    have truth:" - (2 ^ (size ((scast w\<^sub>1)::64 Word.word) - 1)) 
      ...
      Word.word_size[of "(scast w\<^sub>1)::64 Word.word"]
      Word.word_size[of "(scast (0x7FFFFFFF))::64 Word.word"]
      scast_eq1 scast_eq2 sints64 sints32 thing2 thing1 thing3)
    have case1a:"sint (((scast w\<^sub>1)::64 Word.word) + (0x7FFFFFFF::64 Word.word)) 
               = sint ((scast w\<^sub>1)::64 Word.word) + sint (0x7FFFFFFF::64 Word.word)"
      by(rule signed_arith_sint(1)[OF truth])
    have case1b:"sint ((0xFFFFFFFF80000001)::64 Word.word) < 0"
    ...
    have c:"sint (((scast w\<^sub>1)::64 Word.word) + 0x7FFFFFFF) 
          = sint ((scast w\<^sub>1)::64 Word.word) + sint ((0x7FFFFFFF)::64 Word.word)"
      using g case1a 
      by blast
    have d:"sint ((0x7FFFFFFF)::64 Word.word) = (0x7FFFFFFF)" by auto 
    ...
    have case1:"scast POS_INF <=s ?sum \<Longrightarrow> POS_INF \<equiv>\<^sub>L r\<^sub>1 + r\<^sub>2"
      ...
      apply(auto)
          using case1a case1b 
          apply (auto simp add: NEG_INF_def POS_INF_def neq1 eq2 neq3 repINT repL_def 
            repe.simps repeInt_simps lo2 word_sless_alt) 
      using repINT repU_def repe.simps repeInt_simps lo2 word_sless_alt
     proof -
        fix r'
        have h3:"sint (((scast w\<^sub>1)::64 Word.word) + 0x7FFFFFFF) 
          = sint (((scast w\<^sub>1)::64 Word.word)) + sint(0x7FFFFFFF::64 Word.word)" 
          using case1a by auto 
        have h4:"sint(0x7FFFFFFF::64 Word.word) = 2147483647" by auto
        assume a1:"0 \<le> sint ((scast w\<^sub>1)::64 Word.word)"
    ...
    have case2:"\<not>(scast POS_INF <=s ?sum) \<Longrightarrow> scast ?sum \<equiv>\<^sub>L r\<^sub>1 + r\<^sub>2"
      ...
      subgoal
        ...
        have downcast:"sint ((scast (((scast w\<^sub>1)::64 Word.word) + (( 0x7FFFFFFF))))::word) 
          ...
          using scast_down_range[OF rightSize]
          by auto
        then have b:"sint ((scast (((scast w\<^sub>1)::64 Word.word) + 0x7FFFFFFF))::word)
                   = sint (((scast w\<^sub>1)::64 Word.word) + 0x7FFFFFFF)" 
          using g by auto
        show "r'\<^sub>1 + 2147483647 
          = (real_of_int (sint ((scast (((scast w\<^sub>1)::64 Word.word) + 0x7FFFFFFF))::word)))"
          using a b c d e f 
          proof -
            have "(real_of_int (sint ((scast w\<^sub>1)::64 Word.word ) + 2147483647)) 
                = r'\<^sub>1 +  (real_of_int 2147483647)"
      ...
      subgoal proof -
        ...
        from d d2 have d3:"- 2147483647 < sint w\<^sub>1 + sint (0x7FFFFFFF::64 Word.word)"
          by auto
        have d4:"sint ((scast (((scast w\<^sub>1)::64 Word.word) + 0x7FFFFFFF))::word) 
          = sint w\<^sub>1 + sint (0x7FFFFFFF::64 Word.word)"
...
proof -
  ...
  have truth:" - (2 ^ (size ((scast w\<^sub>1)::64 Word.word) - 1)) 
    ...
    \<and> sint ((scast w\<^sub>1)::64 Word.word) + sint ((scast w\<^sub>2)::64 Word.word) 
      \<le> 2 ^ (size ((scast w\<^sub>1)::64 Word.word) - 1) - 1"
    using Word.word_size[of "(scast w\<^sub>2)::64 Word.word"] Word.word_size[of "(scast w\<^sub>1)::64 Word.word"]
    using scast_eq1 scast_eq2 sints64 sints32
      Word.word_sint.Rep[of "(w\<^sub>1)::32 Word.word"]
      Word.word_sint.Rep[of "(w\<^sub>2)::32 Word.word"]
    by auto 
  have sint_eq:"sint((scast w\<^sub>1 + scast w\<^sub>2)::64 Word.word) = sint w\<^sub>1 + sint w\<^sub>2"
    ...
    by auto
  have bigOne:"scast w\<^sub>1 + scast w\<^sub>2 <=s ((- 0x7FFFFFFF)::64 Word.word) 
    \<Longrightarrow> \<exists>r'\<le>r\<^sub>1 + r\<^sub>2. r' \<le>  -0x7FFFFFFF"
  proof -
    assume "scast w\<^sub>1 + scast w\<^sub>2 <=s ((- 0x7FFFFFFF)::64 Word.word)"
    then have sum_leq:"sint w\<^sub>1 + sint w\<^sub>2 \<le> - 0x7FFFFFFF"
          and sum_leq':" (sint w\<^sub>1 + sint w\<^sub>2) \<le>  (- 2147483647)"
  ...
  have bigTwo:
   "\<not>(((scast POS_INF)::64 Word.word) <=s ?sum) \<Longrightarrow> 
    \<not>(?sum <=s ((scast NEG_INF)::64 Word.word)) \<Longrightarrow> 
    \<exists>r'\<le>r\<^sub>1 + r\<^sub>2. r' = (real_of_int (sint (scast (((scast w\<^sub>1)::64 Word.word) 
                      + ((scast w\<^sub>2)::64 Word.word))::word))) 
               \<and> (r' <  0x7FFFFFFF \<and>  (-0x7FFFFFFF) < r')"
  proof -
    assume "\<not>(((scast POS_INF)::64 Word.word) <=s ?sum)"
    ...
    show ?thesis
      ...
      apply(rule conjI)
      subgoal 
        using r12_sint_scast cast_eq leq_ref r2w1 r1w1 add_mono[of r'\<^sub>1 r\<^sub>1 r'\<^sub>2 r\<^sub>2] bound1 bound2
        by auto
  ...
  qed
  have bigThree:"0x7FFFFFFF <=s ((scast w\<^sub>1)::64 Word.word) + ((scast w\<^sub>2)::64 Word.word) 
    \<Longrightarrow> \<exists>r'\<le>r\<^sub>1 + r\<^sub>2. 2147483647 \<le> r'"
  proof -
    ...
    have somethingA:"r'\<^sub>1  \<le>  sint w\<^sub>1" and somethingB:"r'\<^sub>2  \<le>  sint w\<^sub>2" 
      using bound1 bound2 unfolding repe.simps POS_INF_def NEG_INF_def apply auto
      using \<open> 0x7FFFFFFF <=s scast w\<^sub>1 + scast w\<^sub>2 \<close> word_sle_def notinf1 notinf2 
      unfolding POS_INF_def
      by auto
    have something:"r'\<^sub>1 + r'\<^sub>2 \<le>  sint w\<^sub>1 + sint w\<^sub>2"
  ...
  have inf_case:"((scast POS_INF)::64 Word.word) <=s ?sum \<Longrightarrow> POS_INF \<equiv>\<^sub>L r\<^sub>1 + r\<^sub>2"
    proof -
      assume "((scast POS_INF)::64 Word.word) 
          <=s ((scast w\<^sub>1)::64 Word.word) + ((scast w\<^sub>2)::64 Word.word)"
  ...
  have int_case:"\<not>(((scast POS_INF)::64 Word.word) <=s ?sum) 
    \<Longrightarrow> \<not> (?sum <=s ((scast NEG_INF)::64 Word.word)) 
    \<Longrightarrow> ((scast ?sum)::word) \<equiv>\<^sub>L r\<^sub>1 + r\<^sub>2"
  proof -
    assume bound1:"\<not> ((scast POS_INF)::64 Word.word) <=s scast w\<^sub>1 + scast w\<^sub>2"
    assume bound2:"\<not> scast w\<^sub>1 + scast w\<^sub>2 <=s ((scast NEG_INF)::64 Word.word)"
    obtain r'::real  
  ...
  show "(let sum = ?sum in
    ...
    else scast sum) \<equiv>\<^sub>L r\<^sub>1 + r\<^sub>2"
  ...
  subgoal for r
    ...
    subgoal for ra
      proof -
        ...
        have eqNeg:"wmax w\<^sub>1 w\<^sub>2 = POS_INF"
          using p1 i2 bound2b p1 
          unfolding NEG_INF_def POS_INF_def wmax.simps word_sless_def word_sle_def
          by auto 
        have neg_eq:"POS_INF \<equiv>\<^sub>E max r\<^sub>1 r\<^sub>2"
          apply (rule repPOS_INF)
    ...
    subgoal for ra
      proof -
        ...
        have res1:"max r\<^sub>1 r\<^sub>2 =  (real_of_int (sint (wmax w\<^sub>1 w\<^sub>2)))"
          using bound1a bound1b bound2 i1 leD less_trans n2 wmax.simps NEG_INF_def 
            of_int_less_iff word_sless_alt
...
apply(cases "w1 = POS_INF \<and> w2 <s 0") 
  subgoal proof -
    ...
    from eq2 have rw2:"r2 =  (real_of_int (sint w2))" 
      using repe.simps neq1 neq2 neq3 neq4 NEG_INF_def POS_INF_def neq1 neq2 neq3 neq4 theCase 
      by auto 
    show ?thesis
      ...
      apply(auto)
      using upper lower1 lower2 POS_INF_def NEG_INF_def rw2 
    ...
    proof -
      ...
      then show "r1 *  (real_of_int (sint w2)) \<le>  (- 2147483647)"
        using f5 f4 a1 a2 a3 mult_left_mono
...
apply(cases "w1 = POS_INF \<and> 0 = w2") subgoal 
  ...
  show ?thesis
    ...
    apply(auto)
    using upper lower1 POS_INF_def NEG_INF_def rw2 
    by (auto simp add: NEG_INF_def POS_INF_def word_sless_def word_sle_def)
  qed 
...
proof -
  ...
  from neq1 neq2 neq3 neq4 neq5 neq6 theCase
  have w2NotPinf:"w2 \<noteq> POS_INF" and w2NotNinf:"w2 \<noteq> NEG_INF" by auto
  from eq2 theCase  have rw2:"r2 = 0" 
    using repe.simps neq1 neq2 neq3 neq4 NEG_INF_def POS_INF_def neq1 neq2 neq3 neq4 theCase
    by auto
  show ?thesis
    using neq1 neq2 neq3 neq4 neq5 neq6 neq7 neq8 theCase apply simp
    ...
    apply(rule disjI2)
    apply(rule exI[where x="0"])
    using POS_INF_def NEG_INF_def  rw2 
    by (auto simp add: NEG_INF_def POS_INF_def word_sless_def word_sle_def)
  qed
...
proof -
  ...
  using repe.simps neq1 neq2 neq3 neq4 NEG_INF_def POS_INF_def neq1 neq2 neq3 neq4 theCase by auto 
    ...
    apply(rule exI[where x="r1 * r2"])
    apply(auto)
    using upper lower1 lower2 POS_INF_def NEG_INF_def rw1
    apply (auto simp add: NEG_INF_def POS_INF_def word_sless_def word_sle_def)
  proof -
...
proof -
  ...
  have mylem:"\<And>x y z::real. y < 0 \<Longrightarrow>   x \<le> y \<Longrightarrow> z \<le> -1 \<Longrightarrow> -y \<le> x * z"
    ...
    have h4:"-x * -z \<ge> -y"
      ...
      using dual_order.trans mult_left_mono  mult.right_neutral mult.commute
...
from neq1 neq2 neq3 neq4 neq5 neq6 theCase
  ...
  show ?thesis
    using neq1 neq2 neq3 neq4 neq5 neq6 neq7 neq8 neq9 neq10 neq11 neq12 neq13 neq14 theCase 
    apply simp
    unfolding repe.simps
    apply(rule disjI2)
    apply(rule disjI1)
...
proof -
  ...
  assume theCase:"0 = w1 \<and> w2 = NEG_INF"
    from neq1 neq2 neq3 neq4 neq5 neq6 theCase
  have w2NotPinf:"w1 \<noteq> POS_INF" and w2NotNinf:"w1 \<noteq> NEG_INF" by auto
  from eq1 theCase have rw2:"r1 = 0" 
    using repe.simps neq1 neq2 neq3 neq4 NEG_INF_def POS_INF_def neq1 neq2 neq3 neq4 theCase 
    by auto
  show ?thesis
    using neq1 neq2 neq3 neq4 neq5 neq6 neq7 neq8 theCase apply simp
...
proof -
  ...
  have case1:"?prod <=s ((scast NEG_INF)::64 Word.word) \<Longrightarrow> NEG_INF \<equiv>\<^sub>E (r1 * r2)"
    ...
    qed
  have case2:"\<not>(?prod <=s ((scast NEG_INF)::64 Word.word)) 
      \<Longrightarrow> ((scast POS_INF)::64 Word.word) <=s ?prod 
      \<Longrightarrow> POS_INF \<equiv>\<^sub>E (r1 * r2)"
    proof -
      ...
      have neqs:"w1 \<noteq> POS_INF" " w1 \<noteq> NEG_INF" "w2 \<noteq> POS_INF" "w2 \<noteq> NEG_INF"
        ...
        POS_INF_def NEG_INF_def word_sless_def signed.not_less_iff_gr_or_eq by force+
      from eq1 have rw1:"r1 =  (real_of_int (sint w1))" 
        using repe.simps neq1 neq2 neq3 neq4 NEG_INF_def POS_INF_def neqs by auto
      from eq2 have rw2:"r2 =  (real_of_int (sint w2))" by (auto simp add: repe.simps neqs)
    ...
    qed
  have case3:"\<not>(?prod <=s ((scast NEG_INF)::64 Word.word)) 
  \<Longrightarrow> \<not>((scast POS_INF)::64 Word.word) <=s ?prod \<Longrightarrow> ((scast ?prod)::word) \<equiv>\<^sub>E (r1 * r2)"
    proof -
      have h1:"sint ((scast POS_INF)::64 Word.word) = sint POS_INF"
        apply(rule Word.sint_up_scast)
        unfolding Word.is_up by auto
  ...
  subgoal 
    using case1 neq1 neq2 neq3 neq4 neq5 neq6 neq7 neq8 neq9 neq10 neq12 neq13 neq14 neq16 neq15 
      neq17
    unfolding wtimes.simps by presburger
  apply(cases "(((scast POS_INF)::64 Word.word) <=s ?prod)")
  subgoal 
    using case2 neq1 neq2 neq3 neq4 neq5 neq6 neq7 neq8 neq9 neq10 neq12 neq13 neq14 neq16 neq15 
      neq17 
    unfolding wtimes.simps by presburger
  using case3 neq1 neq2 neq3 neq4 neq5 neq6 neq7 neq8 neq9 neq10 neq12 neq13 neq14 neq16 neq15 neq17
  unfolding wtimes.simps by presburger
qed
...
text\<open>Case analysis principle for pairs of intervals\<close>
lemma ivl_zero_case:
...
lemma case_ivl_zero
[case_names One Two ZeroZero ZeroPos ZeroNeg PosZero NegZero NegNeg NegPos PosNeg PosPos]:
  fixes l1 u1 l2 u2 :: real
  shows
...
text\<open>Correctness of multiplication upper bound\<close>
lemma tu_lemma:
...
proof -
  ...
  show ?thesis
  proof(induct taking: ru1 ru2 rl1 rl2 rule: case_ivl_zero)
    case ZeroZero
    assume "rl1 \<le> 0 \<and> 0 \<le> ru1 \<and> rl2 \<le> 0 \<and> 0 \<le> ru2"
    then have geq1:"ru1 \<ge> 0" and geq2:"ru2 \<ge> 0" by auto
    consider "r\<^sub>1 \<ge> 0 \<and> r\<^sub>2 \<ge> 0" | "r\<^sub>1 \<ge> 0 \<and> r\<^sub>2 \<le> 0" | "r\<^sub>1 \<le> 0 \<and> r\<^sub>2 \<ge> 0" | "r\<^sub>1 \<le> 0 \<and> r\<^sub>2 \<le> 0"
    using le_cases by auto
    then show "tu l\<^sub>1 u\<^sub>1 l\<^sub>2 u\<^sub>2 \<equiv>\<^sub>U r\<^sub>1 * r\<^sub>2" 
    proof (cases)
      case 1
      ...
      have up:"ru1 * ru2 \<ge> r\<^sub>1 * r\<^sub>2"
        by auto
      show ?thesis
      using up eru1 eru2 erl1 erl2 max_repU2[OF maxU12] max_repU2[OF maxU34] max_repU2[OF bigMaxU]
  ...
  next
    ...
    consider "r\<^sub>1 \<ge> 0" | "r\<^sub>1 \<le> 0" using le_cases by (auto)
    then show ?thesis
    proof (cases)
      case 1
  ...
  next
    ...
    consider "r\<^sub>2 \<ge> 0" | "r\<^sub>2 \<le> 0" using le_cases by auto
    then show ?thesis
    proof (cases)
      case 1
      have g1:"ru1 * ru2 \<ge> ru1 * r\<^sub>2" 
  ...
  next
    ...
    have up:"ru1 * rl2 \<ge> r\<^sub>1 * r\<^sub>2"
      by auto
    show ?thesis
      using up maxU12 maxU34 bigMaxU wmax.elims max_repU2 max_repU2[OF maxU12] max_repU2[OF maxU34] 
        max_repU2[OF bigMaxU] maxt34 
      by (metis max_repU1 repU_def timesul tu.simps)
  next
    case PosNeg
    ...
    show "tu l\<^sub>1 u\<^sub>1 l\<^sub>2 u\<^sub>2 \<equiv>\<^sub>U r\<^sub>1 * r\<^sub>2"
      using up maxU12 maxU34 bigMaxU wmax.elims max_repU2 max_repU2[OF maxU12] max_repU2[OF maxU34] 
        max_repU2[OF bigMaxU] max.coboundedI1 max.commute maxt34 
      by (metis repU_def tu.simps)
  next
    case PosPos
...
proof -
  ...
  proof(induct taking:ru1 ru2 rl1 rl2 rule: case_ivl_zero)
    ...
    consider "r\<^sub>1 \<ge> 0 \<and> r\<^sub>2 \<ge> 0" | "r\<^sub>1 \<le> 0 \<and> r\<^sub>2 \<ge> 0" | "r\<^sub>1 \<ge> 0 \<and> r\<^sub>2 \<le> 0" | "r\<^sub>1 \<le> 0 \<and> r\<^sub>2 \<le> 0"
      using le_cases by auto
    then show ?thesis
    proof (cases)
      case 1
      ...
      have up:"rl1 * ru2 \<le> r\<^sub>1 * r\<^sub>2"
        by auto
      show ?thesis
        using up eru1 eru2 erl1 erl2 min_repU1 min_repU2 repL_def repU_def timeslu tl.simps 
          wmin.elims
    ...
    next
      ...
      from g1 and g2
      have up:"ru1 * rl2 \<le> r\<^sub>1 * r\<^sub>2" by auto
      show ?thesis
  ...
  next
    ...
    proof (cases)
      ...
      then have g1:"ru1 * rl2 \<le> ru1 * r\<^sub>2" 
        using "1" bounds grl1 grl2 gru1 gru2 mult_le_cancel_left
        by fastforce
      have p1:"\<And>a::real. (0 \<le> - a) = (a \<le> 0)"
        by(auto)
      have p2:"\<And>a b::real.  (- a \<le> - b) = (b \<le> a)" by auto
      have g2:"ru1 * r\<^sub>2 \<le> r\<^sub>1 * r\<^sub>2"
        using "1" r2 bounds grl1 grl2 gru1 gru2 p1 p2
        by (simp add: mult_right_mono_neg)
      from g1 and g2
      have up:"ru1 * rl2 \<le> r\<^sub>1 * r\<^sub>2"
        by auto
    ...
    next
      ...
      have g2:"0 \<le> r\<^sub>1 * r\<^sub>2"
        using "2" r2 zero_le_mult_iff by blast  
      from g1 and g2
      ...
      have g2:"ru1 * r\<^sub>2 \<le> r\<^sub>1 * r\<^sub>2"
        ...
        proof -
          have "\<forall>e ea. r\<^sub>2 * ea \<le> r\<^sub>2 * e \<or> \<not> e \<le> ea" 
            using "2" by (metis mult_left_mono_neg)
          then show ?thesis
...
text\<open>negation of word\<close>
...
text\<open>Compare > 2s-complement representation of words\<close>
fun wgreater :: "word \<Rightarrow> word \<Rightarrow> bool"
where "wgreater w\<^sub>1 w\<^sub>2 = (sint w\<^sub>1 > sint w\<^sub>2)"

lemma neg_less_contra:"\<And>x. Suc x < - (Suc x) \<Longrightarrow> False"
  by auto

text\<open>Comparison < is correct\<close>
lemma wgreater_lemma:"w\<^sub>1 \<equiv>\<^sub>L (r\<^sub>1::real) \<Longrightarrow> w\<^sub>2 \<equiv>\<^sub>U r\<^sub>2 \<Longrightarrow> wgreater w\<^sub>1 w\<^sub>2 \<Longrightarrow> r\<^sub>1 > r\<^sub>2"
  unfolding repU_def repL_def
  apply(auto simp add: POS_INF_def NEG_INF_def )
  subgoal for r'\<^sub>1 r'\<^sub>2 
    proof -
      assume sint_le:"sint w\<^sub>1 > sint w\<^sub>2"
      then have sless:"(w\<^sub>2 <s w\<^sub>1)" using word_sless_alt by auto
...
where "wgeq w\<^sub>1 w\<^sub>2 =
((\<not> ((w\<^sub>2 = NEG_INF \<and> w\<^sub>1 = NEG_INF)
  \<or>(w\<^sub>2 = POS_INF \<and> w\<^sub>1 = POS_INF))) \<and>
(sint w\<^sub>2 \<le> sint w\<^sub>1))"

text\<open>Comparison $\geq$ of words is correct\<close>
lemma wgeq_lemma:"w\<^sub>1 \<equiv>\<^sub>L r\<^sub>1 \<Longrightarrow> w\<^sub>2 \<equiv>\<^sub>U (r\<^sub>2::real) \<Longrightarrow> wgeq w\<^sub>1 w\<^sub>2 \<Longrightarrow> r\<^sub>1 \<ge> r\<^sub>2"
  unfolding wgeq.simps
  subgoal 
    proof -
      assume assms:"\<not> (w\<^sub>2 = NEG_INF \<and> w\<^sub>1 = NEG_INF \<or> w\<^sub>2 = POS_INF \<and> w\<^sub>1 = POS_INF) \<and> sint w\<^sub>2 \<le> sint w\<^sub>1"
      assume a1:"w\<^sub>1 \<equiv>\<^sub>L r\<^sub>1" and a2:"w\<^sub>2 \<equiv>\<^sub>U (r\<^sub>2::real)"
      from assms have sint_le:"sint w\<^sub>2 \<le> sint w\<^sub>1" by(auto simp add: POS_INF_def NEG_INF_def )
      then have sless:"(w\<^sub>2 <=s w\<^sub>1)" using word_sless_alt word_sle_def by auto
...
text\<open>Formula semantics\<close>
...
proof (induction rule: rtsem.induct)
 case rtsem_Const 
   ...
   show "Rep_bword w \<equiv>\<^sub>E r \<Longrightarrow> \<nu> REP \<nu>' \<Longrightarrow> [Const w]<>\<nu> \<equiv>\<^sub>P r"
     using pu_lemma tu_lemma wmax_lemma wmin_lemma  wneg_lemma repU_def repL_def repP_def repe.simps 
       rep_simps repstate_simps order_refl wtsemU.simps(1) represents_state.cases 
     by auto
next
 case rtsem_Var
   fix x \<nu>'
   show "\<nu> REP \<nu>' \<Longrightarrow> [Var x]<>\<nu> \<equiv>\<^sub>P \<nu>' x"
     by(auto simp add: Interval_Arithmetic.wVarU case_prod_conv pu_lemma tu_lemma wmax_lemma 
         wmin_lemma wneg_lemma repU_def repL_def repP_def repe.simps rep_simps repstate_simps 
         order_refl wtsemU.simps(1) represents_state.cases)
next
 case rtsem_Plus
...
next
  ...
  have lbound:"wmax l1 (wneg u1) \<equiv>\<^sub>L (abs r)"
    ...
    apply(rule conjI)  
    using lrep lu repL_def snd_conv wmax_lemma real_minus_le_minus lu repL_def  urep wneg_lemma
     apply linarith
    using lrep lu repL_def snd_conv wmax_lemma real_minus_le_minus lu repL_def  urep wneg_lemma
    by simp  
...
case (wGreaterT t1 v t2 w)
  ...
  note rep1 = trm_sound[of t1 w r\<^sub>1, where \<nu>=v, OF eval1 rep]
  note rep2 = trm_sound[of t2 w r\<^sub>2, where \<nu>=v, OF eval2 rep]
  show "[Greater t1 t2]w \<downharpoonright> True"
    apply(rule rGreaterT[where r\<^sub>1 = r\<^sub>1, where r\<^sub>2 = r\<^sub>2]) 
    prefer 3
    apply(rule wgreater_lemma[where w\<^sub>1="fst([t1]<> v)", where w\<^sub>2="snd([t2]<> v)"])
...
next
  ...
  note rep1 = trm_sound[of t1 v' r\<^sub>1, where \<nu>=v, OF eval1 rep]
  note rep2 = trm_sound[of t2 v' r\<^sub>2, where \<nu>=v, OF eval2 rep]
  show "[Greater t1 t2]v' \<downharpoonright> False"
    apply(rule rGreaterF [of t1 v' r\<^sub>1 t2 r\<^sub>2])
    apply(rule eval1)
     apply(rule eval2)
    apply(rule wgeq_lemma[where w\<^sub>1="fst([t2]<> v)", where w\<^sub>2="snd([t1]<> v)"])
...
lemma rep_upd:"\<omega> = (\<nu>(Inr x := snd([\<theta>]<>\<nu>)))(Inl x := fst([\<theta>]<>\<nu>)) \<Longrightarrow> \<nu> REP \<nu>' \<Longrightarrow> ([\<theta>::trm]\<nu>' \<down> r)
   \<Longrightarrow> \<omega> REP \<nu>'(x := r)"
  apply(rule REPI)
  apply(rule conjI)
