...
lemma case_times_inf
 [case_names 
  ...
  AllFinite]:
(*  HiHi HiZero HiLo ZeroHi ZeroZero ZeroLo LoHi LoZero LoLo*)
fixes w1 w2 P
...
next
  case NegNum
    ...
    have case1:"?sum <=s ((scast NEG_INF)::64 Word.word) \<Longrightarrow> NEG_INF \<equiv>\<^sub>U r1 + r2"
      ...
      proof -
        fix r'
        assume a1:"sint ((scast w2)::64 Word.word) \<le> 0"
        then have h3:"sint w2 \<le> 0" using scast_eq3 by auto 
        assume a2:"r1 \<le> r'"
        assume a3:"r2 \<le>  (real_of_int (sint w2))"
        assume a4:"r' \<le>  (- 2147483647)"
        assume a5:"sint w2 < 2147483647"
        assume a6:"- 2147483647 < real_of_int (sint w2)"
        assume a7:"sint (scast w2 + 0xFFFFFFFF80000001) = sint (scast w2) - 2147483647"
        from a2 a4 have h1:"r1 \<le> - 2147483647" by auto
        from a1 a3 h3 have h2:"r2 \<le> 0" 
    ...
    have case2:"\<not>(?sum <=s scast NEG_INF) \<Longrightarrow> scast ?sum \<equiv>\<^sub>U r1 + r2"
      ...
      subgoal
        proof -
         ...
         have b1:"sint((scast w2)::64 Word.word) = sint w2" 
           apply(rule Word.sint_up_scast)
           unfolding Word.is_up by auto
        ...
        have e:"sint ((scast w2)::64 Word.word) = sint w2" 
        ...
        show "r'\<^sub>2 - 2147483647 
          ...
          proof -
            have "(real_of_int (sint ((scast w2)::64 Word.word) - 2147483647)) 
      ...
      have letRep:"(let sum = ?sum in if sum <=s scast NEG_INF then NEG_INF else scast sum) \<equiv>\<^sub>U r1 + r2"
        by(cases "?sum <=s scast NEG_INF"; (auto simp add: case1 case2)) 
...
next
  ...
  have case1:"?sum <=s ((scast NEG_INF)::64 Word.word) \<Longrightarrow> NEG_INF \<equiv>\<^sub>U r1 + r2"
    ...
    proof -
      ...
      assume a4:"r' \<le>  (- 2147483647)"
      assume a5:"sint w1 < 2147483647"
      assume a6:"- 2147483647 < real_of_int (sint w1)"
      assume a7:"sint (scast w1 + 0xFFFFFFFF80000001) = sint (scast w1) - 2147483647"
  ...
  have case2:"\<not>(?sum <=s scast NEG_INF) \<Longrightarrow> scast ?sum \<equiv>\<^sub>U r1 + r2"
    ...
    proof -
      ...
      have rightSize:"sint (((scast w1)::64 Word.word) + - 0x7FFFFFFF) \<in> sints (len_of TYPE(32))"
        ...
        apply(auto simp add: sints32 len32[of "TYPE(32)"])
        defer
        subgoal using Word.word_sint.Rep[of "(w1)::32 Word.word"] 
          by (auto simp add: sints32 len32[of "TYPE(32)"])
        using w2bound by auto
      have arith:"\<And>x::int. x \<le> 2 ^ 31 - 1 \<Longrightarrow> x + (- 2147483647) < 2147483647"
        by auto
...
next
  ...
  have scast_eq1:"sint((scast w1)::64 Word.word) = sint w1" 
    apply(rule Word.sint_up_scast)
    unfolding Word.is_up by auto
  have scast_eq2:"sint((scast w2)::64 Word.word) = sint w2"
    apply(rule Word.sint_up_scast)
    unfolding Word.is_up by auto
  ...
  have truth:" - (2 ^ (size ((scast w1)::64 Word.word) - 1)) 
   ...
   \<and> sint ((scast w1)::64 Word.word) + sint ((scast w2)::64 Word.word) 
     \<le> 2 ^ (size ((scast w1)::64 Word.word) - 1) - 1"
    using Word.word_size[of "(scast w2)::64 Word.word"] Word.word_size[of "(scast w1)::64 Word.word"]
...
next
  ...
  have case1:" (((scast POS_INF)::64 Word.word) <=s ?sum) \<Longrightarrow> POS_INF \<equiv>\<^sub>L r1 + r2"
   ...
   proof -
      ...
      assume a5:"sint w2 < 2147483647"
      assume a6:"- 2147483647 < real_of_int (sint w2)"
      assume a7:"sint (scast w2 + 0x7FFFFFFF) = sint (scast w2) + 2147483647"
  ...
  have case2:"\<not>(scast POS_INF <=s ?sum) \<Longrightarrow> scast ?sum \<equiv>\<^sub>L r1 + r2"
    ...
    subgoal
      proof -
       ...
       have rightSize:"sint (((scast w2)::64 Word.word) +  0x7FFFFFFF) \<in> sints (len_of TYPE(32))"
         ...
         apply (auto simp add: sints32 len32[of "TYPE(32)"])
         using Word.word_sint.Rep[of "(w2)::32 Word.word"] sints32 len32[of "TYPE(32)"]
         using w2bound by auto
      have arith:"\<And>x::int. x \<le> 2 ^ 31 - 1 \<Longrightarrow> x + (- 2147483647) < 2147483647"
        by auto
      have downcast:"sint ((scast (((scast w2)::64 Word.word) + (( 0x7FFFFFFF))))::word) 
    ...
    proof -
      ...
      have "sint (scast (((scast w2)::64 Word.word) + (0x7FFFFFFF))::word) < 2147483647"
        ...
        using arith[of "sint w2", OF w2bound]
        by auto
    ...
    subgoal proof -
      ...
      have "- 2147483648 \<noteq> w2"
        ...
        using int_not_undef[of w2] unfolding NEG_INF_def 
        by simp
...
next
  case NumPos
    ...
    have "sint (( w1)) \<ge> (-(2 ^ 31))"
      using Word.word_sint.Rep[of "(w1)::32 Word.word"] Word.word_size[of "(scast w1)::64 Word.word"]
    ...
    have case1:"scast POS_INF <=s ?sum \<Longrightarrow> POS_INF \<equiv>\<^sub>L r1 + r2"
     ...
     proof -
        ...
        then have h3:"sint w1 \<ge> 0" using scast_eq3 h3 h4 using scast_eq3 h3 h4 by auto 
    ...
    have case2:"\<not>(scast POS_INF <=s ?sum) \<Longrightarrow> scast ?sum \<equiv>\<^sub>L r1 + r2"
      ...
      proof -
        ...
        have rightSize:"sint (((scast w1)::64 Word.word) + 0x7FFFFFFF) \<in> sints (len_of TYPE(32))"
          ...
          apply (auto simp add: sints32 len32[of "TYPE(32)"])
          defer
          using Word.word_sint.Rep[of "(w1)::32 Word.word"] sints32 len32[of "TYPE(32)"]
          using w2bound by auto
        have arith:"\<And>x::int. x \<le> 2 ^ 31 - 1 \<Longrightarrow> x + (- 2147483647) < 2147483647"
          by auto
        have downcast:"sint ((scast (((scast w1)::64 Word.word) + 0x7FFFFFFF))::word) 
        ...
        show "sint (scast (((scast w1)::64 Word.word) + 0x7FFFFFFF)::word) < 2147483647"
          using downcast d e c arith[of "sint w1", OF d2]
      ...
      subgoal proof -
        ...
        have "- 2147483648 \<noteq> w1"
          ...
          subgoal unfolding POS_INF_def by auto
          subgoal unfolding NEG_INF_def by auto
          using int_not_undef[of w1] unfolding NEG_INF_def 
          by simp
      ...
      have "(let sum = ?sum in if scast POS_INF <=s sum then POS_INF else scast sum) \<equiv>\<^sub>L r1 + r2"
        by (auto simp add: Let_def case1 case2)
      then show ?thesis 
        using neq1 eq2 neq3 by (auto simp add: POS_INF_def NEG_INF_def)
next
  case NumNum
  assume notinf1:"w1 \<noteq> POS_INF"
  assume notinf2:"w2 \<noteq> POS_INF"
  ...
  subgoal for r
    ...
    subgoal for ra
      proof -
        ...
        assume bound1:"r \<le>  (real_of_int (sint NEG_INF))"
        have eqNeg:"max r1 r2 =  (real_of_int (sint ra))"
        ...
        then have extra_eq:"(wmax w1 w2) = ra"
...
proof (cases rule: case_times_inf[of w1 w2])
  ...
  and  a2: "PosInf \<le> r2" 
    using "PosPos" eq1 eq2 repe.simps by (auto simp add: POS_INF_def NEG_INF_def)
  ...
  then have "r1 \<le> r1 * r2"
    ...
    using eq1 eq2 by (auto simp add: POS_INF_def NEG_INF_def repe.simps)
...
next
  ...
  have a1: "NegInf \<ge> r1"
    using eq1 "NegPos" by (auto simp add: POS_INF_def NEG_INF_def repe.simps)
  have a2: "PosInf \<le> r2"
    using eq2 "NegPos" by (auto simp add: POS_INF_def NEG_INF_def repe.simps)
  have f1: "real_of_int Numeral1 = 1"
    by simp
  have f3: "(real_of_int 3) \<le> - r1"
    using a1 by (auto simp add: NEG_INF_def)
  have f4:"0 \<le> r2"
    using f1 a2 by(auto simp add: POS_INF_def)
  have f5: "r1 \<le>  - 1"
    using f3 by auto
  have fact:"r1 * r2 \<le> - r2"
    using f5 f4  mult_right_mono by fastforce
  show ?thesis
    apply (clarsimp simp add: repe.simps "NegPos")
    apply(rule conjI)
    subgoal by auto
    unfolding NEG_INF_def
    apply simp
    using a1 a2 fact 
    by(auto simp add: POS_INF_def)
next
  case PosNeg
  have a1: "PosInf \<le> r1"
    using eq1 "PosNeg" by (auto simp add: POS_INF_def NEG_INF_def repe.simps)
  then have h1:"r1 \<ge> 1"
    by (auto simp add: POS_INF_def)
  have a2: " NegInf \<ge> r2"
    using eq2 "PosNeg" by (auto simp add: POS_INF_def NEG_INF_def repe.simps)
  ...
  have "r1 * r2 \<le> NegInf"
    ...
    by(auto simp add: NEG_INF_def POS_INF_def)
  then show ?thesis
    using "PosNeg" by (auto simp add: NEG_INF_def POS_INF_def repe.simps)
next
  case NegNeg
  have a1: "(-2147483647) \<ge> r1"
    using eq1 "NegNeg" by (auto simp add: POS_INF_def NEG_INF_def repe.simps)
  then have h1:"r1 \<le> -1"
    using  max.bounded_iff max_def one_le_numeral
    by auto
  have a2: " (-2147483647) \<ge> r2"
    using eq2 "NegNeg" by (auto simp add: POS_INF_def NEG_INF_def repe.simps)
  ...
  then have fact:" 2147483647 \<le> r2 * r1"
    ...
    show ?thesis
      apply (clarsimp simp add: repe.simps "NegNeg")
      apply(rule conjI)
      subgoal by auto
...
next
  case PosLo
  from "PosLo"
  have w2NotPinf:"w2 \<noteq> POS_INF" and w2NotNinf:"w2 \<noteq> NEG_INF" by (auto simp add: POS_INF_def NEG_INF_def)
  from eq1 "PosLo" 
...
next
  ...
  from eq2 have rw2:"r2 =  (real_of_int (sint w2))" 
    using repe.simps "PosZero" NEG_INF_def POS_INF_def 
    by auto 
  have "0 = r1 * r2"
...
next
  ...
  subgoal 
    using AllFinite case1 by(auto simp add: repe.simps NEG_INF_def POS_INF_def)
  apply(cases "(((scast POS_INF)::64 Word.word) <=s ?prod)")
  subgoal 
    using AllFinite case2 by(auto simp add: repe.simps NEG_INF_def POS_INF_def)
  using AllFinite case3 by(auto simp add: repe.simps NEG_INF_def POS_INF_def)
qed
  ...
  next
    ...
    next
      ...
      have g2:"ru1 * r2 \<le> r1 * r2"
        using r1 "2" bounds bound grl1 grl2 gru1  gru2
        proof -
          have "\<forall>e ea. r2 * ea \<le> r2 * e \<or> \<not> e \<le> ea" 
