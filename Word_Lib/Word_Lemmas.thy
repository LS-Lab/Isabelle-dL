...
lemma upto_enum_step_shift:
  ...
  ([p , p + 2 ^ m .e. p + 2 ^ n - 1])
      = map (\<lambda> x. x+p) [0, 2 ^ m .e. 2 ^ n - 1]"
...
lemma i_hate_words:
  ...
  apply (rule notI, drule arg_cong[where f="\<lambda> x. x + 1"])
  apply simp
  done
...
lemma word_div_mult:
  ...
  apply (erule order_less_le_trans)
  apply (subst unat_word_ariths)
  using Divides.mod_less_eq_dividend
  by blast
...
lemma int_sdiv_negated_is_minus1 [simp]:
  ...
  apply (rule iffI)
   ...
   apply (clarsimp simp: sign_simps not_less sgn_mult)
   using eq_iff minus_zero neg_le_iff_le 
   apply (smt pos_imp_zdiv_nonneg_iff)
...
lemma sdiv_int_range:
  ...
  apply (metis pos_imp_zdiv_nonneg_iff zero_less_abs_iff)
...
lemma card_map_elide:
  ...
  apply (subst card_insert_disjoint)
   ...
   apply (subgoal_tac "(of_nat::nat \<Rightarrow> 'a::len word) ` {..<n} = {..<of_nat n}")
    ...
    apply clarsimp
    apply (subst (asm) card_word)
  using  Divides.mod_less_eq_dividend order_less_le_trans unat_of_nat
                 word_less_nat_alt
  apply (metis unat_less_helper)
   by clarsimp+
...
lemma map_bits_rev_to_bl:
  "map (\<lambda>y.  x !! y) [0..<size x] = rev (to_bl x)"
